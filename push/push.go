...
func (m *Manager) Append(r *http.Request) (err error) {
	...
	for _, ts := range pb.TimeSeries {
		...
		shouldTrace := false
		if trace.IsTranceOn() {
			if trace.ShouldTrace(&ls) {
				shouldTrace = true
				level.Info(m.logger).Log("msg", "reported series", "label", ls.String())
			}
		}
		if len(ls) == 0 {
		...
		for _, sample := range ts.Samples {
			...
			if !ok {
				...
				if err != nil {
					level.Error(m.logger).Log("msg", "add failed", "label", labelStr, "time", sample.Timestamp, "value", sample.Value)
					return err
				}
				m.lck.Lock()
				m.cache[labelStr] = id
			...
			if shouldTrace {
				level.Info(m.logger).Log("msg", "add sample", "label", labelStr, "time", sample.Timestamp, "value", sample.Value)
			}
		}
	}
	return nil
